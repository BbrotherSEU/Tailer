...
public class Tailer implements Runnable {
    ...
    /**
     * A semaphore to detect whether the tailer is running.
     * @author fanbin
     */
    private Semaphore semaphore;
    
    /**
     * Current tailer position.
     * @author fanbin
     */
...
//                        last = System.currentTimeMillis();
    ...
    /**
     * Read new lines.
     * @author fanbin 修改支持读取utf-8编码的string
     *
    ...
    private long readLines(RandomAccessFile reader) throws IOException {
        ...
        while (run && ((num = reader.read(inbuf)) != -1)) {
            int bufferPosStart = 0;
            for (int i = 0; i < num; i++) {
                byte ch = inbuf[i];
                switch (ch) {
                    case '\n':
                        seenCR = false; // swallow CR before LF
                        bytes = concatByteArrays(bytes, 0, bytes.length, inbuf, bufferPosStart, i - bufferPosStart);
                        bufferPosStart = i + 1;
                        listener.handle(new String(bytes, "UTF-8"));
                        bytes = new byte[0];
                        rePos = pos + i + 1;
                        break;
                    case '\r':
            ...
            if (bufferPosStart < num){
                bytes = concatByteArrays(bytes, 0, bytes.length, inbuf, bufferPosStart, num - bufferPosStart);
            }

            pos = reader.getFilePointer();

            //add by fanbin
            //文件读到末尾返回读取到的位置 -> 改为 -> 每读完一次buffer就更新一次读取到的位置
            //目的是尽可能及时更新日志读取到的位置
            this.position = rePos;
        }
        //如果\r结尾,有可能缺少一行
    ...
    private byte[] addCRToByteArray(byte[] a) {
    ...
    /**
     * Stop the thread and Wait until stopped.
     * @author fanbin
     * @throws InterruptedException
     */
    ...
    /**
     * Set the tailer position, must set when tailer is not running.
     * @author fanbin
     */
